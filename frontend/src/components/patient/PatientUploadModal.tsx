...
const PATIENT_DOC_TYPES: { value: DocumentType; key: string }[] = [
...
const PatientUploadModal: React.FC<PatientUploadModalProps> = ({ onClose, onUpload }) => {
  ...
  const handleSubmit = async () => {
    ...
    setUploading(true);
    const ok = await onUpload({ file, title, type });
    setUploading(false);
    ...
    <ModalOverlay onClick={onClose}>
      <ModalContainer onClick={e => e.stopPropagation()}>
        <ModalHeader>
          <ModalTitle>
            <Upload />
            {t('documents.patientUpload.title')}
          </ModalTitle>
        ...
        <ModalBody>
          {success && (
            <SuccessMessage>
              <CheckCircle />
              {t('documents.patientUpload.success')}
            </SuccessMessage>
          )}
          ...
          <FormGroup>
            ...
            <FormInput
              ...
              placeholder={t('documents.patientUpload.titlePlaceholder')}
            />
          </FormGroup>
        ...
        <ModalFooter>
          ...
          >
            {uploading ? (
              ...
              <>
                <Upload />
                {t('documents.patientUpload.send')}
              </>
            )}
          </ModalButton>
        </ModalFooter>
